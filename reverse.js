...
async function tradingStrategy() {
  ...
  while (true) {
    try {
      ...
      // REVERSE BUY logic: enter when price drops below SMA offset
      if (canTrade && !bought && currentPrice < lowerBand) {
        ...
        console.log(`REVERSE BUY at ${entryPrice}, Tx: ${txHash}, Gas: ${gasFeeETH}, Fee: ${buyFee.toFixed(6)}`, formatIST(new Date()));
      }
      ...
      if (bought) {
        ...
        if (shouldTakeProfit || shouldExit) {
          ...
          const reason = shouldTakeProfit ? 'Reverse Take-Profit' : 'Exit on SMA-recovery';
      ...
      // Re-enable trading when price returns to neutral range
