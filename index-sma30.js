...
async function tradingStrategy() {
    ...
    const duration = 300000; // 5 minutes interval
    ...
    while (true) {
      try {
        ...
        // Use 30-period moving average instead of 5
        const ma30 = calculateMovingAverage(priceHistory, 30);
        if (!ma30) {
          await new Promise(resolve => setTimeout(resolve, duration));
          continue;
        }
  
        console.log(`Price: ${currentPrice}, MA30: ${ma30}`);
  
        if (!positionOpen && currentPrice > ma30) {
          const txHash = await executeTrade('BUY', tradeAmountUSDC);
          console.log(`Buy executed at ${currentPrice}. Tx: ${txHash}`);
          positionOpen = true;
        } else if (positionOpen && currentPrice < ma30) {
