...
async function tradingStrategy() {
  ...
  let waitLongReentry = false;
  let waitShortReentry = false;
  ...
  while (true) {
    try {
      const currentPrice = await getCurrentPrice();
      console.log('CurrentPrice WETH/USDC:', 1 / currentPrice, '-', formatIST(new Date()));
      ...
      console.log(`Price: ${1 / currentPrice}, MA: ${1 / ma}, Offset Range: (${1 / offsetUpper} - ${1 / offsetLower})`);
      ...
      // === LONG ENTRY ===
      if (!longOpen && currentPrice > offsetUpper && !bought && !sold && !shortOpen && !waitLongReentry) {
      ...
      if (longOpen && bought) {
        ...
        if ((takeProfit || stopLoss) && longTP < maxProfitCount) {
          ...
          const swapFeeRate = parseFloat(FEE_PERCENT) / 1_000_000;
          const buySwapFeeUSDC = buyValue * swapFeeRate;
          const sellSwapFeeUSDC = sellValue * swapFeeRate;
      ...
      // === SHORT ENTRY ===
      if (!shortOpen && currentPrice < offsetLower && !sold && !bought && !longOpen && !waitShortReentry) {
